...
const addEventFormSchema: ZodType<IAddEventFormInput> = z.object({
    name: z.string().refine((value) => value !== '', 'Please enter a name for the event.'),
...
export const AddEventForm = (): JSX.Element => {
    const { register, handleSubmit, formState: { errors, isSubmitSuccessful } } = useForm<IAddEventFormInput>({
    ...
    return (
        <div className={clsx('mx-auto flex justify-center items-center rounded-lg bg-base-100 drop-shadow-lg px-6 py-4')}>
            <form className={clsx('max-h-lvh ')} onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className={clsx('flex justify-center items-center gap-4')}>
                    ...
                    <div className={clsx('flex flex-col')}>
                        <TimeInput errorMessage={errors.start?.message} topLeftLabel='Start:' inputProps={register('start', { required: true })} />
                        <TimeInput errorMessage={errors.end?.message} topLeftLabel='End:' inputProps={register('end', { required: true })} />
                    </div>
                </div>
